...
public class ShopService {
    ...
    public Long openOrder(Long userdId) {
        if (authenticator.isAuthenticated(userdId)) {
            Long maxorder = (long) orders.stream()
                    .mapToInt(o -> o.getOrderId().intValue())
                    .max().orElse(0);
            orders.add(new Order(maxorder + 1, userdId, productService));
            return maxorder + 1;
        } else {
            return -1L;
        }
    ...
    public boolean verifyOrder(Long orderId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            Random generator = new Random();
            if (!theOrder.isVeriffied()) {
                theOrder.setVeriffied((result && generator.nextBoolean()));
            }
            return theOrder.isVeriffied();
        }
        return false;
    }
